...
class LearnPage {
    constructor(jsonContent = '[]') {
        this._entries = JSON.parse(jsonContent).map(entry => {
            ...
            return new Entry(version, entry.isAntora, entry.referenceDocUrl, entry.apiDocUrl, entry.current);
        });
    }
    ...
    _sortAndMarkCurrent() {
        this._entries.sort((a, b) => compareVersions(b.version.toString(), a.version.toString()));
